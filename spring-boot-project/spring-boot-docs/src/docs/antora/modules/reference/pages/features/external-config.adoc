...
| `on-profile`
| A profile expression that must match for the document to be active. Such expression can include the NOT operator `!` to allow for any profile but this one. e.g. `!dev` will activate properties for all profiles but 'dev'.
